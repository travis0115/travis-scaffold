...
集群部署时，所有实例必须连接同一套业务数据库，并保持相同的 `scheduler-name` 和 `tablePrefix`；`instanceId` 保持 `AUTO`。项目启动时将 JVM 默认时区设置为 UTC，Cron、单次执行时间、日历限制和日志时间均按应用默认时区解释。
...
处理器发布后，在管理后台的“运维管理 / 任务调度”中创建任务。新建、复制和导入的任务默认处于停用状态，确认配置无误后再启用。
...
当前实现只支持以下 JSON Schema 子集，不是完整 JSON Schema 标准实现：

- `type`：`object`、`array`、`string`、`integer`、`number`、`boolean`、`null`；
- 对象的 `required` 和 `properties`；
- 数组的 `items`。

后台立即执行时可以临时传入参数；不传则使用任务默认参数。临时参数同样会经过任务 Schema 校验，但不会覆盖已保存的默认参数。
...
优先级范围为 1～10，默认值为 5。它只在多个 Trigger 同时等待有限调度线程时影响 Quartz 的选择顺序，不代表业务抢占或严格执行顺序。
...
- 排除日期，例如 `2026-01-01`；
- 排除星期，Quartz 使用 `1=星期日`、`2=星期一`、…、`7=星期六`；
- 每日允许时间段，开始时间和结束时间必须同时填写，且开始时间早于结束时间。

这些限制会组合应用。日历时间同样使用应用默认时区。
...
每个任务可以配置日志保留天数，默认 30 天。系统每天 03:00 物理清理过期执行日志；也可以在执行日志页面按任务或全部清理。调度对账还会把节点中断后遗留的 `RUNNING` 日志标记为中断。
...
应用启动时会立即执行一次对账，运行期间每分钟在集群内限频对账一次。对账根据任务配置指纹检查 JobDetail、Trigger 和 Calendar，并修复缺失、过期或孤立的 Quartz 数据。因此不要直接修改 `qrtz_` 表，也不要把 Quartz 表当作业务配置来源。
...
| 权限码 | 用途 |
| --- | --- |
| `ops:job:query` | 查询任务、处理器、负责人选项、预览、统计、看板和导出 |
| `ops:job:update` | 新增、修改、删除、复制、导入任务 |
| `ops:job:operation` | 启停、立即执行和清理执行日志 |
| `ops:job:log:query` | 查询、查看和导出执行日志 |

权限同时在后端接口校验，前端按钮隐藏不能替代权限控制。负责人和失败告警接收人还会按当前管理员的数据范围校验。

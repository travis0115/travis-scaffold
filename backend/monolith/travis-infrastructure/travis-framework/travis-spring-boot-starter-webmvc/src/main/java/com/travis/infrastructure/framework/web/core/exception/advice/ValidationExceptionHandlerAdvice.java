...
public class ValidationExceptionHandlerAdvice {
    ...
    public ApiResponse<?> handleConstraintViolationException(ConstraintViolationException ex) {
        ...
        var errorMessage = CollUtil.isEmpty(constraintViolations) ? "" :
    ...
    public ApiResponse<?> handleBindException(BindException ex) {
        log.warn("参数绑定异常：", ex);
        var errorMsg = ex.getFieldErrors().stream()
    ...
    public ApiResponse<?> handleMissingServletRequestParameterException(MissingServletRequestParameterException ex) {
        log.warn("请求参数缺失：", ex);
        return ApiResponse.error(CommonErrorCode.VALIDATE_MISSING_SERVLET_REQUEST_PARAMETER, ex.getParameterName());
    }

    /**
     * SpringMVC 路径变量缺失
     * <p>
     * 例如说，接口上设置了 @PathVariable("id") 参数，结果请求路径中未包含该变量
     */
    @ExceptionHandler(MissingPathVariableException.class)
    public ApiResponse<?> handleMissingPathVariableException(MissingPathVariableException ex) {
        log.warn("请求路径变量缺失：", ex);
        return ApiResponse.error(CommonErrorCode.VALIDATE_MISSING_PATH_VARIABLE, ex.getVariableName());
    }
    ...
    public ApiResponse<?> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException ex) {
        ...
        return ApiResponse.error(CommonErrorCode.VALIDATE_METHOD_ARGUMENT_TYPE_MISMATCH, ex.getName());
    }
    ...
    public ApiResponse<?> handlerMethodArgumentTypeInvalidFormatException(HttpMessageNotReadableException ex) {
        log.warn("请求参数类型错误：", ex);
        if (ex.getCause() instanceof InvalidFormatException invalidFormatException) {
            return ApiResponse.error(CommonErrorCode.VALIDATE_METHOD_ARGUMENT_TYPE_MISMATCH,
    ...
    public ApiResponse<?> handleMethodArgumentNotValidExceptionException(MethodArgumentNotValidException ex) {
        log.warn("参数校验失败：", ex);
        // 获取 errorMessage

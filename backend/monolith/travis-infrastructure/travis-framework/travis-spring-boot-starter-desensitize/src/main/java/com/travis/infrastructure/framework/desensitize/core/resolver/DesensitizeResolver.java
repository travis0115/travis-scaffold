...
public class DesensitizeResolver {
    ...
    /** SpEL 上下文 非 Spring 环境或容器未就绪：降级为无 SpEL，所有 disable 不生效 */
    private static class ProviderHolder {
        ...
        private static EvaluationContextProvider getProviderOrNull() {
            try {
                ...
                log.debug(
                        "EvaluationContextProvider not available (non-Spring or context not ready), disable "
    ...
    public static DesensitizeRule resolveRule(Annotation annotation) {
        ...
        var key =
                new DesensitizeRuleCacheKey(
                        annotation.annotationType(),
                        AnnotationAttributeSnapshotBuilder.buildSnapshot(
                                annotation, Collections.newSetFromMap(new IdentityHashMap<>())));
        return RULE_CACHE.computeIfAbsent(
                key,
                _ ->
                        resolveRuleDeep(
                                annotation, Collections.newSetFromMap(new IdentityHashMap<>())));
    ...
    private static RegexDesensitizeRule buildRegexRule(
            Annotation sourceAnnotation, RegexDesensitize meta) {
        var regex = resolveAnnotationField(sourceAnnotation, "regex", meta.regex(), String.class);
        var replacer =
                resolveAnnotationField(sourceAnnotation, "replacer", meta.replacer(), String.class);

        return RegexDesensitizeRule.of(
                regex, replacer, sourceAnnotation.annotationType().getSimpleName());
    }

    private static SliderDesensitizeRule buildSliderRule(
            Annotation sourceAnnotation, SliderDesensitize meta) {
    ...
    private static <T> T resolveAnnotationField(
            Annotation annotation, String fieldName, T defaultValue, Class<T> type) {
        try {
            var handle =
                    DesensitizeMethodHandleCache.getHandle(annotation.annotationType(), fieldName);
            if (handle == null) {
                return defaultValue;
            }
    ...
    /** Spel disable 统一解析 若 EvaluationContextProvider 不可用（非 Spring 环境或容器未就绪），视为不禁用。 */

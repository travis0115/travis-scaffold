...
public interface SysMessageInboxApi {
    ...
    List<SysUserMessageResp> listRecent(
            @NotBlank(message = "接收端不能为空")
                    ...
                    String receiverType,
            @NotNull(message = "用户ID不能为空") @Positive(message = "用户ID必须为正数") Long userId,
            @Min(value = 1, message = "查询数量不能小于1") @Max(value = 50, message = "查询数量不能超过50")
                    Integer limit);
    ...
    SysUserMessageResp getAndMarkRead(
            @NotBlank(message = "接收端不能为空")
                    ...
                    String receiverType,
            @NotNull(message = "用户ID不能为空") @Positive(message = "用户ID必须为正数") Long userId,
            @NotNull(message = "消息ID不能为空") @Positive(message = "消息ID必须为正数") Long id);

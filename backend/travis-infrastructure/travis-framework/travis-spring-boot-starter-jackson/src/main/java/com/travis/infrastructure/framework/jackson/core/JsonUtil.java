...
/**
 * JSON 工具类 ObjectMapper 由 Spring 注入后写入静态 holder，未注入时使用默认实例。
 *
 * <p>线程安全设计：
 *
 * <ul>
 *   <li>{@link #objectMapper} 使用 volatile 修饰，保证多线程环境下的可见性，防止指令重排序导致读取到未完全初始化的对象
 *   <li>所有静态方法都是无状态的，可以安全并发调用
 * </ul>
 */
@Slf4j
public class JsonUtil {
     ...
     *
     * <ol>
     *   <li><strong>禁止指令重排序</strong>：防止在构造函数初始化时，先赋值引用再完成对象初始化，导致其他线程读取到半初始化的对象
     *   <li><strong>保证内存可见性</strong>：一个线程写入的 ObjectMapper 对其他线程立即可见
     * </ol>
     *
     * <p>典型场景：Spring 容器启动时通过构造函数注入 ObjectMapper，业务线程并发调用静态方法使用该实例
     */
     ...
     *
     * <p>注意：该方法是包访问权限，只能被同一包内的 Spring 配置类调用
     *
     ...
     *
     * <p>如果 Spring 尚未注入（如单元测试环境），则返回默认的 ObjectMapper 实例
     *
    ...
    public static <T> T parseObject(String text, Type type) {
        ...
        try {
            return getObjectMapper()
                    .readValue(text, getObjectMapper().getTypeFactory().constructType(type));
        } catch (Exception e) {
            log.error("json parse err, text length:{}", text.length(), e);
            throw new RuntimeException(e);
        }

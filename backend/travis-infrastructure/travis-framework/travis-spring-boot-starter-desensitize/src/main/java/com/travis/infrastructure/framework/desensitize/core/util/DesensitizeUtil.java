...
/** 统一脱敏 API 即时用 toDesensitizedJson，日志延时用 ofDesensitizedJson */
@Slf4j
public class DesensitizeUtil {
    ...
    /**
     * 从 Spring 容器懒加载并缓存，由容器中 DesensitizeObjectSerializer Bean 提供实现。 容器未就绪或未注册该 Bean 时为
     * null，toDesensitizedJson 退化为 obj.toString()。
     */
    ...
    /** 用注解类的默认值构建脱敏规则。 例如 resolveDefaultRule(MobileDesensitize.class) → SliderRule(3,4,'*') */
    public static <A extends Annotation> DesensitizeRule resolveDefaultRule(
